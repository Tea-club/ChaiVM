...
chai::bytecode_t instr2RawRR(Operation op, RegisterId r1, RegisterId r2) {
    return (operation2opcode(op)) | (r1 << 8) | (r2 << 16);
}

chai::bytecode_t instr2RawI(Operation op, Immidiate imm) {
    return (operation2opcode(op)) | (static_cast<chai::bytecode_t>(imm) << 16);
}

chai::bytecode_t instr2RawN(Operation op) { return (operation2opcode(op)); }
