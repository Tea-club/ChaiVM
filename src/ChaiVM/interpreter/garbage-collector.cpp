    ...
    while (frame != nullptr) {
        for (int i = 0; i < frame->size(); ++i) {
            if (frame->isRegisterReference(i)) {
                ...
                if (ref != CHAI_NULL) {
                    // @todo #111:60min check if we already collected this root
                    // object.
...
void GarbageCollector::mark() {
    for (auto &root : roots_) {
        // @todo #111:60min multiple roots can refer to the similar object, so
        // this assert will fail even in correct situations.
...
void GarbageCollector::markObjects(Object obj) {
    //    assert(!obj.isMarked());
    obj.mark();
    std::vector<Field> obj_fields =
        exec_.getCodeManager()->getKlass(obj.klassId()).fields_;
...
void GarbageCollector::sweep() {
    ...
    for (auto &alloc_info : allocator.allocations()) {
        ...
        // @todo #111:60min to remove freed allocation here instead of
        // additional loop
